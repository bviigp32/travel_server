...
const generatorSeed = async () => {
  ...
  for (let i = 0; i < 500; i++) {
    ...
    const accommodation_preferences = faker.random
      .arrayElement(["Hotel", "Hostel", "Airbnb"])
      .substring(0, 100);
    const dietary_preferences = faker.random
      .arrayElement(["Vegetarian", "Vegan", "Gluten Free", "No Restrictions"])
      .substring(0, 100);
    const transportation_preferences = faker.random
      .arrayElement(["Car", "Public Transport", "Bike"])
      .substring(0, 100);
    ...
    const health_status = faker.random
      .arrayElement(["No health issues", "Allergies"])
      .substring(0, 255);
